...
// accessible at the global scope so that they can be modified or filtered by other page controls.
...
//### Load data
d3.csv("faraz2.csv", function (data) {
    ...
    var monthlyDimension = ndx.dimension(function (d){
        return d3.time.day(d.dd).getDate();
    });
    // maintain running tallies by day as filters are applied or removed
    var monthlyPerformanceGroup = monthlyDimension.group().reduce(
       ...
       function (p, v) {
                --p.count;
                p.absGain -= v.close - v.open;
                p.fluctuation -= Math.abs(max(v.close) - min(v.open));
                p.sumIndex -= ( v.close)/20 ;
                p.avgIndex = p.sumIndex / p.count;
                p.percentageGain = (p.absGain / p.avgIndex) * 100;
                p.fluctuationPercentage = (p.fluctuation / p.avgIndex) * 100;
       ...
       //* `.radiusValueAccessor` Identifies the value that will be applied agains the `.r()` determine radius size, by default this maps linearly to [0,100]
        ...
        .title(function (p) {
            return [p.key,
                   ...
                   "Fluctuation: " + numberFormat(p.value.fluctuation),
                   "Avg Index: " + numberFormat(p.value.avgIndex),
    ...
    moveChart
        ...
        .x(d3.time.scale().domain([new Date(2013, 11, 31), new Date(2014, 2, 31)]))
    ...
    volumeChart.width(990)
        ...
        .centerBar(true)
        .gap(1)
        .x(d3.time.scale().domain([new Date(2014, 0, 1), new Date(2014, 2, 31)]))
        .round(d3.time.month.round)
        .alwaysUseRounding(true)
        .xUnits(d3.time.months);
